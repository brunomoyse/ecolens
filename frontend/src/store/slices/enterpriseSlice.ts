...
export const enterpriseSlice = createSlice({
	...
	reducers: {
		setSelectedEnterprises: (state, action) => {
			...
			const uniqueEnterprises = action.payload.filter(
				(enterprise: any, index: number, self: Enterprise[]) =>
					index === self.findIndex((t: any) => t.establishment_number === enterprise.establishment_number),
			);
