...
export default function BottomPanel() {
	...
	return (
		...
		>
			<ResizablePanel className="bg-transparent" defaultSize={65} minSize={65} style={{ pointerEvents: 'none' }}>
				<div className="flex h-full bg-transparent -z-10" />
			</ResizablePanel>
			<div
				className="flex justify-between items-center bg-gray-200 w-full py-4 pr-6"
				style={{ pointerEvents: 'auto' }}
			>
				<ResizableHandle withHandle className="py-3" />
				{panelRef.current?.isExpanded() && (
					<PanelBottomClose onClick={togglePanel} className="h-6 w-6 text-black cursor-pointer" />
				)}
				{panelRef.current?.isCollapsed() && (
					<PanelBottomOpen onClick={togglePanel} className="h-6 w-6 text-black cursor-pointer" />
				)}
			</div>

			<ResizablePanel ref={panelRef} collapsible={true} defaultSize={35} maxSize={35}>
