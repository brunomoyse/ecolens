...
export default function CircleInfoCard() {
	...
	const handleExportButtonClick = () => {
		...
		// Export enterprises
		const enterprisesCSV = convertArrayToCSV(
			circleSearchResults.enterprises.map(({ __typename, ...rest }) => rest),
		);
		...
		>
			...
			<div className="px-6 pb-6 pt-2">
				...
				<ul className="list-disc space-y-2">
					<li>
						<strong>PAE:</strong>{' '}
						{circleSearchResults && circleSearchResults.eaps ? circleSearchResults.eaps.length : 0}
					</li>
					<li>
						<strong>Entreprises: </strong>{' '}
						{circleSearchResults && circleSearchResults.enterprises
							? circleSearchResults.enterprises.length
							: 0}
					</li>
					<li>
						<strong>Emplois:</strong> à calculer
					</li>
					<li>
						<strong>Parcelles:</strong>{' '}
						{circleSearchResults && circleSearchResults.plots ? circleSearchResults.plots.length : 0}
					</li>
					<li>
						<strong>Parcelles non-bâties:</strong> à calculer
					</li>
				</ul>

				<Button size="sm" className="w-full mt-4 uppercase" onClick={handleExportButtonClick}>
					Exporter détail
				</Button>
			</div>
		</aside>
